...
Player::Player(std::string name, int health, float speed, COORD position, std::vector<std::vector<SHORT>> texture, KeyboardInputController* inputController)
	...
	audio.loadWav(L"assets\\sounds\\shot.wav", shot);
...
void Player::SetBindings(const KeyBindings& newBindings)
{
...
void Player::Update(float deltaTime) {
	...
	rigidbody.velocity = Vector2(0, 0);
	if (inputController->IsKeyHeld(bindings.moveUp) || inputController->IsKeyPressed(bindings.moveUp)) {
		rigidbody.velocity.y = -1 * speed;
		lastMoveTP = std::chrono::system_clock::now();
	}
	if (inputController->IsKeyHeld(bindings.moveDown) || inputController->IsKeyPressed(bindings.moveDown)) {
		rigidbody.velocity.y = 1 * speed;
		lastMoveTP = std::chrono::system_clock::now();
	}
	if (inputController->IsKeyHeld(bindings.moveLeft) || inputController->IsKeyPressed(bindings.moveLeft)) {
		rigidbody.velocity.x = -1 * speed;
		lastMoveTP = std::chrono::system_clock::now();
	}
	if (inputController->IsKeyHeld(bindings.moveRight) || inputController->IsKeyPressed(bindings.moveRight)) {
		rigidbody.velocity.x = 1 * speed;
		lastMoveTP = std::chrono::system_clock::now();
	}
